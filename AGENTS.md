...
/ (repo root, also the live nginx root for /hkmonitor/)
  ...
  scripts/build_worker_whitelist.py  ← regenerates worker/src/whitelist.generated.js from sources.json
  worker/                ← the Cloudflare Worker (CORS proxy + whitelist + rate limit + edge cache +
                           server-side 3D URL injection). Not deployed yet. `npm run dev` to run locally.
